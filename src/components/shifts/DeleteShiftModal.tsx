...
export function DeleteShiftModal({ open, record, onClose, onConfirm }: DeleteShiftModalProps) {
  const { t, i18n } = useTranslation()
  const isArabic = i18n.language === 'ar'

  if (!record) return null

  const displayName = isArabic ? record.nameAr : record.name

  return (
    <Modal open={open} onClose={onClose} title={t('shifts.delete.title')} size="sm">
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">
          {t('shifts.delete.message', { name: displayName })}
        </p>
