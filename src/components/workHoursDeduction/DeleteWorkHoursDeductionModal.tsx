...
type DeleteWorkHoursDeductionModalProps = {
  open: boolean
...
export function DeleteWorkHoursDeductionModal({
  open,
  record,
  onClose,
  onConfirm,
}: DeleteWorkHoursDeductionModalProps) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t('workHoursDeduction.delete.title')} size="sm">
      <div className="flex flex-col items-center text-center">
        ...
        <p className="text-sm text-slate-600 dark:text-slate-400">
          {t('workHoursDeduction.delete.message', { name: displayName })}
        </p>
      ...
      <div className="mt-6 flex justify-end gap-2 border-t border-slate-200/80 pt-4 dark:border-slate-700/80">
        ...
        >
          {t('workHoursDeduction.delete.confirm')}
        </button>
      </div>
    </Modal>
  )
