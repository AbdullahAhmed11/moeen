...
type ViewWorkHoursAdditionModalProps = {
  open: boolean
  record: WorkHoursAddition | null
  onClose: () => void
}
...
export function ViewWorkHoursAdditionModal({
  open,
  record,
  onClose,
}: ViewWorkHoursAdditionModalProps) {
  ...
  const fields = [
    { label: t('workHoursAddition.table.id'), value: `#${record.id}` },
    {
      label: t('workHoursAddition.table.employee'),
      value: isArabic ? record.employeeNameAr : record.employeeName,
    },
    {
      label: t('workHoursAddition.table.date'),
      value: formatDate(record.date, i18n.language),
    },
    {
      label: t('workHoursAddition.table.addedHours'),
      value: formatWorkHours(record.addedMinutes, i18n.language),
    },
    {
      label: t('workHoursAddition.table.reason'),
      value: isArabic ? record.reasonAr : record.reason,
    },
    {
      label: t('workHoursAddition.table.amount'),
      value: formatCurrency(record.amount, i18n.language),
    },
    {
      label: t('workHoursAddition.table.status'),
      value: <StatusBadge status={record.status} />,
    },
  ]

  return (
    <Modal open={open} onClose={onClose} title={t('workHoursAddition.view.title')} size="md">
